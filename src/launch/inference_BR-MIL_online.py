# src/launch/inference_BR-MIL_online.py
...
def _record_to_dict(rec: Any) -> Dict[str, Any]:
    """
    summarize_repeats_to_records() 可能返回 dataclass (BenchRecord)。
    为了能动态加 profile_* 字段并写 CSV，这里统一转成 dict。
    """
    if rec is None:
        return {}
    ...
    # dataclass (most likely BenchRecord)
...
def _dataset_path_hash8(data_cfg: DataConfig, split_idx: str) -> str:
    """
    MUST match src/data/dataset.py ChunkedCTSDataset hash rule:
      path_hash = md5(f"{data_file_path}|{alignment}").hexdigest()[:8]
    """
...
def _coerce_assemble_cfg(node: Any) -> Any:
    """
    Try hard to convert Hydra/OmegaConf node -> TokenAssembleConfig.
    If conversion fails, keep node as-is (but then we must guard attribute access).
    """
    if node is None:
        try:
...
def _resolve_required_ckpt_path(p: Optional[str], *, run_dir: Path, orig_cwd: Path) -> Path:
    """
    Only resolve explicitly provided ckpt path. No best/last fallback.
    """
    if p is None:
        raise KeyError(
            "[inference_BR-MIL_online] Missing run.checkpoint. "
            "This script does not save ckpt; please pass the EM checkpoint path explicitly."
        )
...
def _load_em_checkpoint_into_models(
    ...
    *,
    ...
    if not isinstance(ckpt, dict):
        raise RuntimeError(f"[inference_BR-MIL_online] checkpoint is not a dict: {ckpt_path}")
    ...
    if cand_agg is None or cand_inst is None:
        raise RuntimeError(
            "[inference_BR-MIL_online] Cannot locate both agg/instance state_dict in ckpt. "
...
class PairIndexAdapter:
    """
    We only need: pair_id -> (start_uid, end_uid) in CTS uid space.
    Also expose correct num_pairs / pair_ids for sampling.
    """
    ...
    def _infer_num_pairs(pi: Any) -> int:
        ...
        if isinstance(pi, dict) and "pair_offsets" in pi:
            off = pi["pair_offsets"]
            return int(len(off) - 1)
        # older formats: try common attrs
        for attr in ("num_pairs", "n_pairs", "N_pairs"):
            if hasattr(pi, attr):
                try:
                    ...
                    pass
        raise RuntimeError("[inference_BR-MIL_online] Cannot infer num_pairs from pair_index.")

    def __len__(self) -> int:
        # IMPORTANT: for select_pair_ids_subset fallback
        return int(self._num_pairs)

    @property
    def pair_ids(self) -> List[int]:
        # IMPORTANT: for select_pair_ids_subset preferred path
...
class BRMILOnlineForward:
    """
    Fig3 Online BR-MIL (compute-only, no caches):
      1) gather all uids for batch (all n)
      2) one-pass gather X/pos/esa for all uids (CPU)  [NO per-uid __getitem__ loop]
      3) cheap forward on all uids (chunked) -> cheap_score(+optional cheap_emb)
      4) selector chooses K per pair (prefer indices to avoid reloading)
      5) build [B,K,C,L] CPU batch by indexing gathered tensors (NO second dataset read)
      6) expensive encode K -> assemble tokens -> aggregator forward
    """

    def __init__(
        self,
        *,
        modules: BRMILOnlineModules,
        cts_ds: ChunkedCTSDataset,
        pair_index: PairIndexAdapter,
        device: torch.device,
        K: int,
        assemble_cfg: Any,
        cheap_chunk_size: int = 8192,
        amp: bool = True,
        ...
        self.cheap_chunk_size = int(cheap_chunk_size)
        self.amp = bool(amp and (device.type == "cuda"))
        self.amp_dtype = amp_dtype

        self._warned_selector_fallback = False
    ...
    def _profile(self, name: str, timer: Any = None):
        # If bench_utils provides a timer with context manager, use it
    ...
    def _batch_gather_all(self, uids_all: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Preferred: vectorized gather (train_em style)
        if hasattr(self.cts_ds, "batch_gather_by_uid"):
            ...
            esa = out.get("esa_scores", out.get("esa", None))

            if pos is None:
                # fallback: some implementations use "positions"
                pos = out.get("positions", None)
            if esa is None:
                esa = out.get("esa_score", None)

            if X is None or pos is None or esa is None:
                raise RuntimeError(
                    "[inference_BR-MIL_online] batch_gather_by_uid missing keys. "
        ...
        # Fallback (slower): per-uid __getitem__ (should rarely be used)
    ...
    def _cheap_forward_all(
        ...
        """
        Inputs: CPU tensors aligned with uids_all order.
        Returns (CPU):
          cheap_score: [M] float32
          cheap_emb:   [M,D] float16 or None
        """
        M = int(X_flat.shape[0])
        if M == 0:
            return torch.empty((0,), dtype=torch.float32), None

        cheap_scores: List[torch.Tensor] = []
        cheap_embs: List[torch.Tensor] = []

        # NOTE: run_benchmark_once is likely already doing autocast.
        # Here we only control input dtype to better reflect AMP vs FP32 throughput.
        in_dtype = self.amp_dtype if self.amp else torch.float32

        with torch.no_grad():
            for s in range(0, M, self.cheap_chunk_size):
                e = min(M, s + self.cheap_chunk_size)
                ...
                if feat is None or logit is None:
                    out = None
                    try:
                        out = self.modules.cheap_model({"X": X, "esa_scores": esa, "pos": pos})
                    except Exception:
                ...
                cheap_scores.append(logit.detach().to(dtype=torch.float32).cpu())
    ...
    def _selector_choose_flat_indices(
        ...
        *,
        ...
        # SelectorModule 是 CPU-only；且 cheap_emb 最好用 float32（CPU half 很容易炸）
        if cheap_emb is not None and cheap_emb.dtype != torch.float32:
            cheap_emb = cheap_emb.to(dtype=torch.float32)

        # 构造 ragged lists（每个 pair 一段）
        ...
        for b in range(B):
            ...
            # 你在 __call__ 里 uids_all 是 arange(start_uid, end_uid)，所以这里直接用 start_uid+range 即可：
            ln = e_seg - s_seg
            ...
            logit_list.append(cheap_score[s_seg:e_seg].to(dtype=torch.float32))
            if cheap_emb is None:
                emb_list.append(None)
            else:
                emb_list.append(cheap_emb[s_seg:e_seg].to(dtype=torch.float32))

        # 关键：真实 selector API（训练时就是这个 contract）
        # mode/epoch/pair_ids 是为了确定性 & reproducibility；Fig3 benchmark 固定即可
        sel_uids_batch_i32, sel_len_i32 = selector.forward_batch(
            uids_list=uids_list,
            pos_list=pos_list,
        ...
        # sel_uids_batch: int32 [B, cfg.kmax] padded -1
        sel_uids_batch = sel_uids_batch_i32.to(dtype=torch.long)  # 便于后面算 index
        ...
        *,
        ...
        if M == 0:
            ...
            out0: Dict[str, Any] = {
                ...
                "mask": torch.zeros((B, K), dtype=torch.bool, device="cpu"),
                "X": torch.zeros((B, K, 1, 1), dtype=torch.uint8, device="cpu"),
            }
            if _assemble_cfg_flag(self.assemble_cfg, "use_pos", False):
                out0["pos"] = torch.zeros((B, K), dtype=torch.float32, device="cpu")
            if _assemble_cfg_flag(self.assemble_cfg, "use_esa", False):
                out0["esa_scores"] = torch.zeros((B, K), dtype=torch.float32, device="cpu")
            return out0
        ...
        idx2d = mask.nonzero(as_tuple=False)  # [N,2]
        if idx2d.numel() > 0:
            flat_idx = sel_flat_idx[idx2d[:, 0], idx2d[:, 1]]
            # safety: filter any unexpected -1
            good = flat_idx >= 0
            if torch.any(~good):
                idx2d = idx2d[good]
                flat_idx = flat_idx[good]

            X_valid = X_flat[flat_idx]  # [N,C,L]
            X_pad[idx2d[:, 0], idx2d[:, 1]] = X_valid
        ...
        out: Dict[str, Any] = {
            "pair_id": torch.tensor(pair_ids, dtype=torch.long, device="cpu"),
            "y_pair": torch.zeros((B,), dtype=torch.float32, device="cpu"),  # compute-only
    ...
    def __call__(self, batch: Dict[str, Any], timer: Any = None, **_: Any) -> torch.Tensor:
        # timer is provided by bench_utils.run_benchmark_once; optional for profiling.
        ...
        # 1) build uids_all (flat) + offsets per pair
        ...
        # 2) one-pass gather X/pos/esa for all uids (CPU)
        with self._profile("B_gather_all_cpu", timer):
            X_flat, esa_flat, pos_flat = self._batch_gather_all(uids_all)

        # 3) cheap forward for selection (CPU outputs)
        with self._profile("C_cheap_forward", timer):
            cheap_score_cpu, cheap_emb_cpu = self._cheap_forward_all(X_flat, esa_flat, pos_flat)

        # 4) selector picks K -> indices into flat arrays
        with self._profile("D_selector", timer):
            sel_flat_idx, mask = self._selector_choose_flat_indices(
                ...
                ends_uid=ends_uid,
                offsets_n=offsets_n,
                cheap_score=cheap_score_cpu,
                cheap_emb=cheap_emb_cpu,
                pos_flat=pos_flat,
                esa_flat=esa_flat,
            )

        # 5) build batch_cpu by indexing gathered tensors (NO second dataset read)
        with self._profile("E_build_batch_cpu_K", timer):
            batch_cpu = self._build_batch_cpu_from_flat(
                pair_ids=pair_ids,
                X_flat=X_flat,
                pos_flat=pos_flat,
                esa_flat=esa_flat,
        ...
        # 6) expensive encode K + assemble tokens + aggregator
        with torch.no_grad():
            with self._profile("F_instance_encode", timer):
            ...
            with self._profile("G_assemble_tokens", timer):
            ...
            with self._profile("H_agg_forward", timer):
                y = self.modules.agg_model(tokens, attn_mask=attn_mask)

        # normalize return: must be tensor
...
def main(cfg: DictConfig) -> None:
    ...
    if bench_csv_cfg is None or str(bench_csv_cfg).strip() in ("", "None", "null"):
        csv_path = bench_dir / "fig3_brmil_online.csv"
    else:
    ...
    # pair_index (MUST match ChunkedCTSDataset naming)
    path_hash8 = _dataset_path_hash8(data_cfg, split)
    print(f"[inference_BR-MIL_online] path_hash8={path_hash8}")

    # use dataset offsets if available; else load pair_index file
    pair_index_raw = None
    if hasattr(cts_ds, "pair_offsets") and getattr(cts_ds, "pair_offsets") is not None:
        ...
        if pair_index_path is None or (not Path(pair_index_path).exists()):
            ...
            raise FileNotFoundError(
                "[inference_BR-MIL_online] pair_index file not found.\n"
    ...
    # bind identity (best-effort): if you re-use an old subset on a different dataset, results become meaningless
    try:
        ...
        print(f"[inference_BR-MIL_online] dataset_identity_hash={ident_hash}")
    except Exception:
        ident_hash = None

    pair_ids_subset = select_pair_ids_subset(
        pi,  # IMPORTANT: pass adapter, not pi.pair_index
    ...
    print(
        f"[inference_BR-MIL_online] split={split} K={K} | "
        ...
        f"cheap_chunk_size={cheap_chunk_size}"
    )
    ...
    # cheap model config
    cheap_arch_cfg = cfg.get("cheap_model", None)
    ...
    # load (inst, agg) from EM ckpt
    ckpt_path_cfg = run_cfg.get("checkpoint", None)  # must be explicit
    ...
    if sel_mod_node is None:
        raise KeyError("[inference_BR-MIL_online] Missing selector config: cfg.em.selector_module")
    ...
    forward_fn = BRMILOnlineForward(
        ...
        cheap_chunk_size=cheap_chunk_size,
        amp=amp,
    ...
    print(f"[inference_BR-MIL_online] CSV appended: {csv_path}")
