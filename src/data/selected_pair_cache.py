...
class _BaseSelectedPairCacheWriter:
    def __init__(
        self, root: str | Path, *, split: str, cache_type: str, meta: SelectedPairCacheMeta
    ):
        self.root = Path(root)
        ...
        self.label = np.memmap(
            self.cache_dir / "label.f32.mmap",
            mode="w+",
            dtype=np.float32,
            shape=(int(meta.num_pairs),),
        )
        self.sel_len = np.memmap(
            self.cache_dir / "sel_len.i16.mmap",
            mode="w+",
            dtype=np.int16,
            shape=(int(meta.num_pairs),),
        )
        self.esa = np.memmap(
            self.cache_dir / "esa.f16.mmap",
            mode="w+",
            dtype=np.float16,
            shape=(int(meta.num_pairs), int(meta.kmax)),
        )
        self.pos = np.memmap(
            self.cache_dir / "pos.f16.mmap",
            mode="w+",
            dtype=np.float16,
            shape=(int(meta.num_pairs), int(meta.kmax)),
        )
...
class SelectedPairCacheWriter(_BaseSelectedPairCacheWriter):
    ...
    def __init__(
        ...
        *,
        ...
        self.X = np.memmap(
            self.cache_dir / "X.u8.mmap", mode="w+", dtype=np.uint8, shape=(P, K, C, L)
        )
        self.X[:] = 0

        self.cheap_logit = None
        if meta.has_cheap_logit:
            self.cheap_logit = np.memmap(
                self.cache_dir / "cheap_logit.f16.mmap", mode="w+", dtype=np.float16, shape=(P, K)
            )
            self.cheap_logit[:] = 0
    ...
    def write_pair(
        ...
        *,
        ...
        if self.cheap_logit is not None and cheap_logit is not None:
            self.cheap_logit[pid, :n] = (
                cheap_logit[:n].detach().cpu().numpy().astype(np.float16, copy=False)
            )
        if self.cheap_emb is not None and cheap_emb is not None:
            self.cheap_emb[pid, :n] = (
                cheap_emb[:n].detach().cpu().numpy().astype(np.float16, copy=False)
            )
    ...
    def __init__(
        ...
        *,
        ...
        self.inst_emb = np.memmap(
            self.cache_dir / "inst_emb.f16.mmap", mode="w+", dtype=np.float16, shape=(P, K, D)
        )
        self.inst_emb[:] = 0
        self.inst_logit = None
        if meta.has_inst_logit:
            self.inst_logit = np.memmap(
                self.cache_dir / "inst_logit.f16.mmap", mode="w+", dtype=np.float16, shape=(P, K)
            )
            self.inst_logit[:] = 0
    ...
    def write_pair(
        ...
        *,
        ...
        if self.inst_logit is not None and inst_logit is not None:
            self.inst_logit[pid, :n] = (
                inst_logit[:n].detach().cpu().numpy().astype(np.float16, copy=False)
            )
...
class SelectedPairCacheReader:
    def __init__(self, root: str | Path, *, split: str, cache_type: str = "selected_raw"):
        ...
        self.label = np.memmap(
            self.cache_dir / "label.f32.mmap", mode="r", dtype=np.float32, shape=(P,)
        )
        self.sel_len = np.memmap(
            self.cache_dir / "sel_len.i16.mmap", mode="r", dtype=np.int16, shape=(P,)
        )
        self.esa = np.memmap(
            self.cache_dir / "esa.f16.mmap", mode="r", dtype=np.float16, shape=(P, K)
        )
        self.pos = np.memmap(
            self.cache_dir / "pos.f16.mmap", mode="r", dtype=np.float16, shape=(P, K)
        )
        ...
        if self.meta.cache_type == "selected_raw":
            ...
            self.X = np.memmap(
                self.cache_dir / "X.u8.mmap", mode="r", dtype=np.uint8, shape=(P, K, C, L)
            )
            if self.meta.has_cheap_logit:
                self.cheap_logit = np.memmap(
                    self.cache_dir / "cheap_logit.f16.mmap",
                    mode="r",
                    dtype=np.float16,
                    shape=(P, K),
                )
            ...
            if self.meta.has_inst_logit:
                self.inst_logit = np.memmap(
                    self.cache_dir / "inst_logit.f16.mmap", mode="r", dtype=np.float16, shape=(P, K)
                )
        else:
    ...
    def read_pair(self, pair_id: int) -> Dict[str, torch.Tensor | int | float | None]:
        ...
        if self.cheap_logit is not None:
            out["cheap_logit"] = torch.from_numpy(
                np.array(self.cheap_logit[pid], dtype=np.float32, copy=True)
            )
        if self.cheap_emb is not None:
            out["cheap_emb"] = torch.from_numpy(
                np.array(self.cheap_emb[pid], dtype=np.float32, copy=True)
            )
        if self.inst_emb is not None:
            out["inst_emb"] = torch.from_numpy(
                np.array(self.inst_emb[pid], dtype=np.float32, copy=True)
            )
        if self.inst_logit is not None:
            out["inst_logit"] = torch.from_numpy(
                np.array(self.inst_logit[pid], dtype=np.float32, copy=True)
            )
        return out
    ...
    *global* ``pair_id`` and the writer internally maps it to a local
    ...
    def __init__(
        ...
        *,
        ...
        self.label = np.memmap(
            self.output_dir / "label.f32.mmap", mode="w+", dtype=np.float32, shape=(S,)
        )
        self.sel_len = np.memmap(
            self.output_dir / "sel_len.i16.mmap", mode="w+", dtype=np.int16, shape=(S,)
        )
        self.esa = np.memmap(
            self.output_dir / "esa.f16.mmap", mode="w+", dtype=np.float16, shape=(S, K)
        )
        self.pos = np.memmap(
            self.output_dir / "pos.f16.mmap", mode="w+", dtype=np.float16, shape=(S, K)
        )
        self.inst_emb = np.memmap(
            self.output_dir / "inst_emb.f16.mmap", mode="w+", dtype=np.float16, shape=(S, K, D)
        )
        self.inst_logit: Optional[np.memmap] = None
        if has_inst_logit:
            self.inst_logit = np.memmap(
                self.output_dir / "inst_logit.f16.mmap", mode="w+", dtype=np.float16, shape=(S, K)
            )
