...
def _check_meta_identity(
    kind: str, meta: Dict[str, Any], *, split: str, path_hash: str, dataset_hash_key: str
) -> None:
    ...
    if str(meta.get("path_hash")) != str(path_hash):
        raise RuntimeError(
            f"[{kind}] meta.path_hash={meta.get('path_hash')} != store.path_hash={path_hash}"
        )
    if str(meta.get("dataset_hash_key")) != str(dataset_hash_key):
        raise RuntimeError(
            f"[{kind}] meta.dataset_hash_key={meta.get('dataset_hash_key')} != store.dataset_hash_key={dataset_hash_key}"
        )
...
def _check_file_size(path: Path, expected_bytes: int, kind: str) -> None:
    ...
    if actual != int(expected_bytes):
        raise RuntimeError(
            f"[{kind}] memmap file size mismatch: {path} actual={actual} expected={expected_bytes}"
        )
...
class FileLock:
    ...
    def __enter__(self):
        ...
        try:
            ...
            raise RuntimeError(
                f"Lock exists: {self.lock_path}. Another process maybe building this cache."
            )
...
class MemmapCacheStore:
        ...
        *,
        ...
        if os.path.exists(self._cheap_meta_path) and not overwrite:
            ...
            if _dtype_str(dtype_logits) != str(meta_dtype_logits):
                raise RuntimeError(
                    f"[CheapCache] dtype_logits mismatch: meta={meta_dtype_logits} requested={_dtype_str(dtype_logits)}"
                )
            if _dtype_str(dtype_emb) != str(meta_dtype_emb):
                raise RuntimeError(
                    f"[CheapCache] dtype_emb mismatch: meta={meta_dtype_emb} requested={_dtype_str(dtype_emb)}"
                )
            ...
            if bool(has_entropy) != meta_has_entropy:
                raise RuntimeError(
                    f"[CheapCache] has_entropy mismatch: meta={meta_has_entropy} requested={bool(has_entropy)}"
                )

            # 文件完整性（存在 + size 正确）
            _check_file_size(
                logits_path, _nbytes(meta_dtype_logits, (int(total_cts),)), "CheapCache"
            )
            _check_file_size(
                emb_path, _nbytes(meta_dtype_emb, (int(total_cts), int(emb_dim))), "CheapCache"
            )
            if meta_has_entropy:
                _check_file_size(
                    entropy_path, _nbytes(meta_dtype_logits, (int(total_cts),)), "CheapCache"
                )

            # 通过校验后再创建 memmap（用 meta dtype 打开）
            self.cheap_meta = CheapMeta(**d)
            self._logits = np.memmap(
                logits_path, mode="r+", dtype=meta_dtype_logits, shape=(total_cts,)
            )
            self._emb = np.memmap(
                emb_path, mode="r+", dtype=meta_dtype_emb, shape=(total_cts, emb_dim)
            )
            self._entropy = (
                np.memmap(entropy_path, mode="r+", dtype=meta_dtype_logits, shape=(total_cts,))
                if meta_has_entropy
                else None
            )
            return
        ...
        with FileLock(lock_path):
            ...
            if has_entropy:
                self._entropy = np.memmap(
                    entropy_path, mode="w+", dtype=dtype_logits, shape=(total_cts,)
                )
            else:
                self._entropy = None
    ...
    def write_cheap_slice(
        ...
        if emb.shape[0] != n or emb.shape[1] != self._emb.shape[1]:
            raise ValueError(
                f"emb shape mismatch: expect ({n},{self._emb.shape[1]}), got {tuple(emb.shape)}"
            )
        ...
        *,
        ...
        if os.path.exists(self._sel_meta_path) and not overwrite:
            ...
            if _dtype_str(dtype_uids) != str(meta_dtype_uids):
                raise RuntimeError(
                    f"[SelectionCache] dtype_uids mismatch: meta={meta_dtype_uids} requested={_dtype_str(dtype_uids)}"
                )
            if _dtype_str(dtype_len) != str(meta_dtype_len):
                raise RuntimeError(
                    f"[SelectionCache] dtype_len mismatch: meta={meta_dtype_len} requested={_dtype_str(dtype_len)}"
                )

            _check_file_size(
                uids_path, _nbytes(meta_dtype_uids, (int(num_pairs), int(kmax))), "SelectionCache"
            )
            _check_file_size(len_path, _nbytes(meta_dtype_len, (int(num_pairs),)), "SelectionCache")

            self.sel_meta = SelectionMeta(**d)
            self._sel_uids = np.memmap(
                uids_path, mode="r+", dtype=meta_dtype_uids, shape=(num_pairs, kmax)
            )
        ...
        with FileLock(lock_path):
            ...
            self._sel_uids = np.memmap(
                uids_path, mode="w+", dtype=dtype_uids, shape=(num_pairs, kmax)
            )
    ...
    def write_selection(
        self, pair_ids: torch.Tensor, sel_uids: torch.Tensor, sel_len: torch.Tensor
    ) -> None:
    ...
    def write_selection_slice(
        self, start_pair_id: int, sel_uids: torch.Tensor, sel_len: torch.Tensor
    ) -> None:
        if self._sel_uids is None or self._sel_len is None:
            raise RuntimeError("SelectionCache not opened.")
    ...
    def read_selection(
        self, pair_ids: Union[List[int], torch.Tensor], K: int
    ...
    def create_or_open_instance(
        ...
        *,
        ...
        if os.path.exists(self._inst_meta_path) and not overwrite:
            ...
            _check_version(
                "InstanceCache", d.get("sel_version_used", ""), sel_version_used, "sel_version_used"
            )
            _check_version(
                "InstanceCache",
                d.get("cheap_version_used", ""),
            ...
            if int(d.get("total_cts")) != int(total_cts) or int(d.get("emb_dim")) != int(emb_dim):
                if not require_ready:
                    ...
                    print(
                        f"[InstanceCache] shape mismatch but require_ready=False, removing stale cache at {self.inst_dir}"
                    )
            ...
            if _dtype_str(dtype_logits) != str(meta_dtype_logits):
                raise RuntimeError(
                    f"[InstanceCache] dtype_logits mismatch: meta={meta_dtype_logits} requested={_dtype_str(dtype_logits)}"
                )
            if _dtype_str(dtype_emb) != str(meta_dtype_emb):
                raise RuntimeError(
                    f"[InstanceCache] dtype_emb mismatch: meta={meta_dtype_emb} requested={_dtype_str(dtype_emb)}"
                )

            _check_file_size(
                logit_path, _nbytes(meta_dtype_logits, (int(total_cts),)), "InstanceCache"
            )
            _check_file_size(
                emb_path, _nbytes(meta_dtype_emb, (int(total_cts), int(emb_dim))), "InstanceCache"
            )

            self.inst_meta = InstanceMeta(**d)
            self._inst_logit = np.memmap(
                logit_path, mode="r+", dtype=meta_dtype_logits, shape=(total_cts,)
            )
            self._inst_emb = np.memmap(
                emb_path, mode="r+", dtype=meta_dtype_emb, shape=(total_cts, emb_dim)
            )
        ...
        with FileLock(lock_path):
            ...
            self._inst_logit = np.memmap(
                logit_path, mode="w+", dtype=dtype_logits, shape=(total_cts,)
            )
            self._inst_emb = np.memmap(
                emb_path, mode="w+", dtype=dtype_emb, shape=(total_cts, emb_dim)
            )
    ...
    def create_or_open_instance_pair_indexed(
        ...
        *,
        ...
        if os.path.exists(self._inst_meta_path) and not overwrite:
            ...
            if (
                meta_num_pairs != int(num_pairs)
                or meta_kmax != int(kmax)
                or meta_emb_dim != int(emb_dim)
            ):
                raise RuntimeError(
            ...
            _check_file_size(
                logit_path, _nbytes(meta_dtype_logits, (int(num_pairs), int(kmax))), "InstanceCache"
            )
            _check_file_size(
                emb_path,
                _nbytes(meta_dtype_emb, (int(num_pairs), int(kmax), int(emb_dim))),
                "InstanceCache",
            )

            self.inst_meta = InstanceMeta(**d)
            self._inst_logit = np.memmap(
                logit_path, mode="r+", dtype=meta_dtype_logits, shape=(num_pairs, kmax)
            )
            self._inst_emb = np.memmap(
                emb_path, mode="r+", dtype=meta_dtype_emb, shape=(num_pairs, kmax, emb_dim)
            )
            return
        ...
        with FileLock(lock_path):
            ...
            self._inst_logit = np.memmap(
                logit_path, mode="w+", dtype=dtype_logits, shape=(num_pairs, kmax)
            )
            self._inst_logit[:] = np.nan
            ...
            if emb_nbytes > 50 * (1024**3):  # > 50GB
                ...
                self._inst_emb = np.memmap(
                    emb_path, mode="r+", dtype=dtype_emb, shape=(num_pairs, kmax, emb_dim)
                )
                # Sparse file provides zeros — no explicit init needed
            else:
                self._inst_emb = np.memmap(
                    emb_path, mode="w+", dtype=dtype_emb, shape=(num_pairs, kmax, emb_dim)
                )
                self._inst_emb[:] = 0
            self.flush_instance()
