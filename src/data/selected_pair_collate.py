...
def selected_pair_collate(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if batch[0].get("inst_emb", None) is not None:
        out["inst_emb"] = torch.stack([x["inst_emb"] for x in batch], dim=0)

    if batch[0].get("inst_logit", None) is not None:
        out["inst_logit"] = torch.stack([x["inst_logit"] for x in batch], dim=0)
