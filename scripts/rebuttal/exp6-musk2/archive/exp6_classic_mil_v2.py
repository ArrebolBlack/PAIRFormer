#!/usr/bin env python3
"""
exp6_classic_mil_v2.py — EXP6D: BR-MIL on Classic MIL Benchmarks (Musk2 + MNIST-Bags)

Fixed version of exp6_classic_mil.py:
- K values adapted for Musk2 (median bag size=12)
- ABMIL no longer loops over K values
- Default epochs=200
- Saves F1/acc in results (not just auc)
- MNIST-Bags bag_lambda=100

- Default K_values includes both small and large K

Usage:
    cd PAIRFormer
    python scripts/rebuttal/exp6_classic_mil_v2.py \
        --datasets musk2 \
        --seeds 2020 2025 2026 \
        --K_values 3 6 10 12 \
        --epochs 200 \
        --output_dir experiments/EXP6D_Musk2/results
"""
...
def generate_mnist_bags(n_bags, bag_lambda, positive_digit=9, seed=42,
    ...
    all_images = dataset.data.float() / 255.0
    all_labels = dataset.targets
    all_features = all_images.view(len(all_images), -1)
...
def main():
    ...
    parser.add_argument("--K_values", nargs="+", type=int, default=[1, 4, 12, 64, 512, 1024])
    parser.add_argument("--epochs", type=int, default=200)
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument("--output_dir", default="experiments/EXP6D_Musk2/results")
    # MNIST-Bags config
    parser.add_argument("--mnist_n_train", type=int, default=500)
    parser.add_argument("--mnist_n_test", type=int, default=100)
    parser.add_argument("--mnist_bag_lambda", type=int, default=100)
        ...
        if dataset_name == "musk2":
            ...
            for seed in args.seeds[:1]:
                ...
                for fold_idx, (train_idx, test_idx) in enumerate(skf.split(range(len(all_bags)), labels)):
                    ...
                    m = run_one("abmil", train_bags, test_bags, 0, seed + fold_idx,
                ...
                if fold_metrics:
                    ...
                    result = {
                        "dataset": dataset_name, "method": "abmil", "K": 0, "seed": seed,
                        "auc_mean": float(np.mean(aucs)), "auc_std": float(np.std(aucs)),
                        "f1_mean": float(np.mean(f1s)), "f1_std": float(np.std(f1s)),
                        "acc_mean": float(np.mean(accs)), "acc_std": float(np.std(accs)),
                        "fold_details": fold_metrics,
                    }
                    tag = f"{dataset_name}/abmil"
                    run_dir = output_dir / tag / f"seed_{seed}"
                    run_dir.mkdir(parents=True, exist_ok=True)
                    with open(run_dir / "metrics.json", "w") as f:
                        json.dump(result, f, indent=2)
                    all_results.append(result)
                    print(f"  {tag}/seed_{seed}: AUC={np.mean(aucs):.4f}+/-{np.std(aucs):.4f}, "
            ...
            for K in args.K_values:
                for seed in args.seeds:
                    ...
                    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(range(len(all_bags)), labels)):
                        train_bags = [all_bags[i] for i in train_idx]
                        test_bags = [all_bags[i] for i in test_idx]
                        m = run_one("brmil", train_bags, test_bags, K, seed + fold_idx,
                                    args.epochs, args.lr, device)
                        if m is not None:
                            fold_metrics.append(m)
                    ...
                    if fold_metrics:
                        ...
                        accs = [m["accuracy"] for m in fold_metrics]
                        result = {
                            "dataset": dataset_name, "method": "brmil", "K": K, "seed": seed,
                            "auc_mean": float(np.mean(aucs)), "auc_std": float(np.std(aucs)),
                            "f1_mean": float(np.mean(f1s)), "f1_std": float(np.std(f1s)),
                            "acc_mean": float(np.mean(accs)), "acc_std": float(np.std(accs)),
                            "fold_details": fold_metrics,
                        }
                        tag = f"{dataset_name}/brmil/K{K}"
                        run_dir = output_dir / tag / f"seed_{seed}"
                        run_dir.mkdir(parents=True, exist_ok=True)
                        with open(run_dir / "metrics.json", "w") as f:
                            json.dump(result, f, indent=2)
            ...
            for method in ["abmil", "brmil"]:
                k_loop = args.K_values if method == "brmil" else [0]
                for K in k_loop:
    ...
    for ds in args.datasets:
        print(f"\n  {ds}:")
        for method in ["abmil", "brmil"]:
            k_values = [0] if method == "abmil" else args.K_values
            for K in k_values:
                matching = [r for r in all_results
                            if r["dataset"] == ds and r["method"] == method and r["K"] == K]
                if not matching:
                    continue
                if "auc_mean" in matching[0]:
                    aucs = [r["auc_mean"] for r in matching]
                    f1s = [r.get("f1_mean", r.get("f1", 0)) for r in matching]
    ...
    csv_path = output_dir / "exp6d_summary.csv"
