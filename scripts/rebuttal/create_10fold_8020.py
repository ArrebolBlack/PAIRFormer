#!/usr/bin/env python3
"""
Create 10-fold 80/20 balanced splits from miRAW Test0-9.
Each fold: 548 pos + 548 neg → 80/20 stratified split.
"""
...
def create_fold_split(fold_id, seed=2020):
    data_path = f"data/miRAW_Test{fold_id}.txt"
    output_dir = f"data/rebuttal/miRAW_fold{fold_id}_split"
    os.makedirs(output_dir, exist_ok=True)

    random.seed(seed + fold_id)
    entries, header = read_data(data_path)

    # Deduplicate by (mirna_id, mrna_id) — keep first entry per pair
    pair_first = {}
    pair_label = {}
    for e in entries:
    ...
    write_data(os.path.join(output_dir, "miRAW_Train_Validation.txt"), out_header, all_train)
    write_data(os.path.join(output_dir, "miRAW_Test.txt"), out_header, test_entries)
